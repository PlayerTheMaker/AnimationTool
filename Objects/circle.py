...
class circle:
    ...
    def update(self,WINDOW,frame,colorshift,camera):
        ...
        draw_ellipse_angle(WINDOW,pygame.Color(int(255*colorshift[0]),int(255*colorshift[1]),int(255*colorshift[2]),a=colorshift[3]),(self.x-abs(self.w)/2+camera[0],self.y-abs(self.h)/2+camera[1],abs(self.w),abs(self.h)),self.r,2)
    ...
    def edit(self,frame,mouse,WINDOW,playing,camera):
        if not playing and frame < len(self.positions):
            pygame.draw.circle(WINDOW,pygame.Color(255,255,100),(self.x+camera[0],self.y+camera[1]),(abs(self.w/3)+abs(self.h/3))/3,6)
            ...
            if abs(mouse[0][0]-self.x-camera[0]) < (abs(self.w/3)+abs(self.h/3))/3+3 and abs(mouse[0][1]-self.y-camera[1]) < (abs(self.w/3)+abs(self.h/3))/3+3:
